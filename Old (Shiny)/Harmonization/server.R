...
#######################
# Reactive function to read in files that are input by users for Quality Control Check
#######################
readInputQCFiles <- reactive({
	uidsfile=input$uids
	refmetfile=input$refmet

	if(is.null(uidsfile)) {
		uids=data.frame()}
	else {
		uids=read.delim(uidsfile$datapath,sep=",",header=T)
	}
	if(is.null(refmetfile)) {
		refmet=data.frame()}
	else {
	    refmet=read.delim(refmetfile$datapath,sep=",",header=T)
	}
	return(list(uids=uids,refmet=refmet))
}) # end readInputQCFiles

#######################
# Reactive function to read in files that are input by users for Harmonization Check
#######################
readInputHarmFiles <- reactive({
	...
	return(list(names=metlist[,1],masteruids=masteruids))
