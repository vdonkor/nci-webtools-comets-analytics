    ...
    appComets.generateHeatmap = function (el, options, xLabels, yLabels, legendLabel, data) {
        ...
        return Plotly.newPlot(el, [{
            ...
            margin: {
                t: 32,
                b: Math.min(50,7*xLabels.map(function(e) { return e.length; }).reduce(function(prev,curr) { return Math.max(prev,curr); },0)),
                l: options.clustered ? 0 : Math.min(250,Math.max(50,7*yLabels.map(function(e) { return e.length; }).reduce(function(prev,curr) { return Math.max(prev,curr); },0)))
            },
            width: options.width,
            shapes: shapes,
            title: " ",
